    ...
    <View style={[styles.nav, isMobile && styles.navMobile, { backgroundColor: colors.background }]}> 
      {isMobile ? (
            ...
            <Modal transparent animationType="fade" onRequestClose={() => setMenuOpen(false)}>
              <TouchableOpacity style={styles.modalBackdrop} onPress={() => setMenuOpen(false)} activeOpacity={1}>
                <View style={styles.mobileMenu}>
                  ...
                  <TouchableOpacity style={styles.link} onPress={() => { setMenuOpen(false); router.push('/leaderboards'); }}>
                    <Text style={[styles.linkText, { color: colors.text }]}>Leaderboards</Text>
                  </TouchableOpacity>
        ...
        <>
          ...
          >
            {({ hovered }) => (
              <Text style={[styles.linkText, { color: hovered ? (theme === 'dark' ? '#fff' : colors.text) : colors.text }]}>Leaderboards</Text>
            )}
          </Pressable>
          <Pressable
